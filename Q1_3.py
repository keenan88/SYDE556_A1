...
if __name__ == "__main__":
    ...
    X_hat = D*A
    Q1_1.plot_decoded_vs_ideal(x_linspace, X_hat, "No Noise No Regularization")
    
    # Noise in A, D not regularized
    ro = 0.2*np.amax(A)
    A_noisy = A + np.random.normal(0, ro, A.shape)
    X_hat_noisy = D * A_noisy
    Q1_1.plot_decoded_vs_ideal(x_linspace, X_hat_noisy, "Noise Without Regularization")
    ...
    Q1_1.plot_decoded_vs_ideal(x_linspace, X_hat_reg, "Regularization without noise")
    Q1_1.plot_decoded_vs_ideal(x_linspace, X_hat_noisy_reg, "Regularization with noise")
